...
int main() {
    ...
    if (archivo_solicitudes == NULL) {
        printf("Error al abrir el archivo solicitudes.txt\n");
        exit(1);
    }
    ...
    FILE *archivo_asignadas = fopen("asignadas.txt", "w");
    if (archivo_asignadas == NULL) {
        printf("Error al crear el archivo asignadas.txt\n");
        exit(1);
    }
    ...
    FILE *archivo_espera = fopen("espera.txt", "w");
    if (archivo_espera == NULL) {
        printf("Error al crear el archivo espera.txt\n");
        exit(1);
    }
    ...
    for (int i = 0; i < num_clientes; i++) {
        ...
        if (entradas_pendientes > 0) {
            fprintf(archivo_espera, "%s,%s,%d\n", cliente->rut, cliente->nombre, entradas_pendientes);
        }
    }
